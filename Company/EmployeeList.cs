...
namespace Company
{
    class EmployeeList : BaseList<Employee>
    {
        ...
        {
            return list.Exists(x => x.Surname == obj.Surname && x.Name == obj.Name && x.MiddleName == obj.MiddleName && x.PositionId == obj.PositionId && x.DepartmentId == obj.DepartmentId);
        }
        public override string ToString()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                if (list[i] != null)
                {
                    sb.AppendFormat(employee, i + 1, list[i].Surname, list[i].Name, list[i].MiddleName, list[i].PositionId, list[i].DepartmentId);
                }
